...
In "x" mode, the pages will only be able to specify the x range, similarly for
...
"plotunselected" events from being fired when the pages clicks the mouse without
dragging.
...
The "plotselected" event is only fired when the pages has finished making the
selection. A "plotselecting" event is fired during the process with the same
parameters as the "plotselected" event, in case you want to know what's
happening while it's happening,

A "plotunselected" event with no arguments is emitted when the pages clicks the
