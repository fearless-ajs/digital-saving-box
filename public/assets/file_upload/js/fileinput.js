...
(function (factory) {
        ...
        _setZoomContent: function ($frame, navigate) {
            ...
            if (desc) {
                if (self.showDescriptionClose) {
                    desc = '<button class="kv-desc-hide">' + self.previewZoomButtonIcons.close + '</button>' + desc;
                }
                $desc.show().html(desc);
                if (self.showDescriptionClose) {
                    self._handler($modal.find('.kv-desc-hide'), 'click', function () {
                        $(this).parent().fadeOut(self.fadeDelay);
                    });
                }
            }
    ...
    $.fn.fileinput.defaults = {
        ...
        resumableUploadOptions: {
            ...
            maxRetries: 3,
            showErrorLog: true,
            retainErrorHistory: true, // display complete error history always unless pages explicitly resets upload
